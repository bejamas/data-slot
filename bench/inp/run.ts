 ...
 *   script  the part of work spent running JS (event handlers, rAF callbacks,
 *           timers, microtasks), including style and layout that JS forces.
 *           This is the share component code controls; the rest is the
 *           browser rendering what changed.
...
type Interaction = { inp: number; work: number; script: number };
const SCRIPT_EVENTS = new Set(["EventDispatch", "FunctionCall", "TimerFire", "FireAnimationFrame", "RunMicrotasks"]);

const overlap = (spans: TraceEvent[], windows: Array<[number, number]>) => {
  let total = 0;
  for (const span of spans) {
    for (const [start, end] of windows) {
      total += Math.max(0, Math.min(end, span.ts + span.dur!) - Math.max(start, span.ts));
    }
  }
  return total / 1000;
};
...
function readInteractions(events: TraceEvent[]): Interaction[] {
  ...
  const mainEvents = events.filter((e) => e.pid === pid && e.tid === mainThread && e.ph === "X" && e.dur);
  const tasks = mainEvents.filter((e) => e.name === "RunTask");
  const scripts: TraceEvent[] = [];
  let scriptEnd = 0;
  for (const e of mainEvents.filter((e) => SCRIPT_EVENTS.has(e.name)).sort((a, b) => a.ts - b.ts)) {
    if (e.ts >= scriptEnd) scripts.push(e);
    scriptEnd = Math.max(scriptEnd, e.ts + e.dur!);
  }
  ...
  return [...byId.keys()].sort((a, b) => a - b).map((id) => {
    ...
    return {
      inp: Math.max(...windows.map(([s, e]) => e - s)) / 1000,
      work: overlap(tasks, merged),
      script: overlap(scripts, merged),
    };
  });
}
...
type ScenarioResult = { init: number; work: number; script: number; inp: number; steps: Interaction[] };
const summarize = (variant: string, script: string) => {
  ...
  for (const [name, { init, steps }] of samples.get(variant)!) {
    const perStep = steps.map((step) => ({
      ...
      script: median(step.map((s) => s.script)),
    }));
    results[name] = {
      init: median(init),
      work: sumOf(perStep.map((s) => s.work)),
      script: sumOf(perStep.map((s) => s.script)),
  ...
  const totals = {
    ...
    script: sumOf(Object.values(results).map((s) => s.script)),
...
console.log(`${"scenario".padEnd(22)}${"steps".padStart(6)}${"script".padStart(16)}${"work".padStart(16)}${"inp".padStart(16)}${"init".padStart(16)}`);
for (const [name, s] of Object.entries(results)) {
  const b = base?.scenarios[name];
  console.log(`${name.padEnd(22)}${String(s.steps.length).padStart(6)}${cell(s.script, b?.script)}${cell(s.work, b?.work)}${cell(s.inp, b?.inp)}${cell(s.init, b?.init)}`);
}
const t = base && {
  work: sumOf(Object.keys(results).map((name) => base.scenarios[name]?.work ?? NaN)),
  script: sumOf(Object.keys(results).map((name) => base.scenarios[name]?.script ?? NaN)),
...
console.log(`${"TOTAL".padEnd(28)}${cell(totals.script, t?.script)}${cell(totals.work, t?.work)}${cell(totals.inp, t?.inp)}${cell(totals.init, t?.init)}`);
